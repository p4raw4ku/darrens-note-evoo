...
```mermaid
flowchart TD
    A["公元前 4500 克里特島栽培起源"] --> B["公元前 2000 邁諾安文明儲油陶罐"]
    B --> C["古希臘 雅典娜神話 奧運塗身 神廟燈油"]
    C --> D["古羅馬 5300萬陶罐 帝國級貿易"]
    D --> E["中世紀 威尼斯熱那亞貿易財富"]
    E --> F["20世紀中 Ancel Keys 七國研究 健康定位"]
    F --> G["21世紀 PREDIMED RCT 精品市場分化"]
```
...
```mermaid
flowchart TD
    A["1 採收 10至12月"] --> B["2 洗果"]
    B --> C["3 搗碎成糊"]
    C --> D["4 攪拌 malaxation 關鍵步驟"]
    D --> E["5 離心分離"]
    E --> F["6 過濾 可選"]
    F --> G["7 儲存 氮氣避光"]
```

> ⭐ **攪拌（malaxation）是最關鍵步驟**——溫度與時間的窄區間決定萃油率與多酚保留（見下方第 4 點）；採收後 12-24 小時內必須完成壓榨。
